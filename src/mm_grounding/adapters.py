...
class QueryTokenEncoder(nn.Module):
    ...
    def __init__(
        ...
        if position_encoding not in {"none", "sinusoidal"}:
            raise ValueError("position_encoding must be 'none' or 'sinusoidal'")
    ...
    def _sinusoidal_positions(
        ...
        frequencies = torch.exp(
            ...
            * (-torch.log(torch.tensor(10000.0, device=device)) / width)
        )
    ...
    def forward(
        ...
        if self.position_encoding == "sinusoidal":
            ...
            tokens = tokens + positions.unsqueeze(0) * attention_mask.unsqueeze(-1).to(
                tokens.dtype
            )
...
class ModalityStageFusion(nn.Module):
    ...
    def __init__(
        ...
        cross_dims = {} if query_dim == hidden_dim else {"kdim": query_dim, "vdim": query_dim}
        self.language_attention = nn.MultiheadAttention(
            hidden_dim, query_attention_heads, batch_first=True, **cross_dims
        )
    ...
    def forward(
        ...
        token_available = torch.ones(
            auxiliary.shape[0], 1, dtype=torch.bool, device=auxiliary.device
        )
        if self.training and self.modality_dropout:
            ...
            token_available = torch.repeat_interleave(keep, repeats, dim=0)
            auxiliary = auxiliary * token_available.to(auxiliary.dtype)
        if query_fusion_scale > 0:
        ...
        return (
            self.residual_scale.to(rgb_tokens.dtype)
            * gate.to(rgb_tokens.dtype)
            * token_available.to(rgb_tokens.dtype)
...
class JointQueryAwareStageFusion(nn.Module):
    ...
    def __init__(
        ...
        zero_init_restore: bool,
        query_dim: int | None = None,
        mixer_dim: int | None = None,
    ) -> None:
        super().__init__()
        query_dim = query_dim or hidden_dim
        mixer_dim = mixer_dim or hidden_dim * 2
        self.modality_dropout = modality_dropout
        self.rgb_projection = nn.Sequential(
            nn.LayerNorm(token_dim), nn.Linear(token_dim, hidden_dim), nn.GELU()
        )
        ...
        self.joint_mixer = nn.Sequential(
            nn.Linear(hidden_dim * 5, mixer_dim),
            nn.GELU(),
            nn.Linear(mixer_dim, hidden_dim),
            nn.GELU(),
        )
    ...
    def _query_context(
        ...
        for index, rgb_chunk in enumerate(torch.split(rgb, patch_counts, dim=0)):
            ...
            if ir_query_tokens is not None and ir_available[index]:
                queries.append(ir_fusion_scale * ir_query_tokens[index])
            if depth_query_tokens is not None and depth_available[index]:
                queries.append(depth_fusion_scale * depth_query_tokens[index])
            if queries:
    ...
    def forward(
        ...
        ir_available = self._availability(
            ir_tokens is not None and ir_fusion_scale > 0, batch_size, rgb.device
        )
        depth_available = self._availability(
            depth_tokens is not None and depth_fusion_scale > 0, batch_size, rgb.device
        )
        ...
        ir = ir * ir_fusion_scale * ir_token_available.unsqueeze(-1).to(ir.dtype)
        depth = (
            depth
            * depth_fusion_scale
            * depth_token_available.unsqueeze(-1).to(depth.dtype)
        )
        ...
        agreements = torch.cat(
            (
                self._cosine(rgb, ir)
                * ir_fusion_scale
                * ir_token_available.unsqueeze(-1),
                self._cosine(rgb, depth)
                * depth_fusion_scale
                * depth_token_available.unsqueeze(-1),
...
class ParallelBackboneFusion(nn.Module):
    ...
    def __init__(
        ...
        fusion_mixer_dim: int | None = None,
        query_dim: int | None = None,
        language_dim: int = 2048,
        num_fusion_stages: int = 4,
        fusion_layer_indices: tuple[int, ...] = (),
        ...
        fusion_dim = fusion_dim or hidden_dim
        fusion_mixer_dim = fusion_mixer_dim or fusion_dim * 2
        query_dim = query_dim or hidden_dim
        fusion_attention_heads = fusion_attention_heads or query_attention_heads
        ...
        if fusion_layer_indices:
            indices = tuple(fusion_layer_indices)
            if tuple(sorted(set(indices))) != indices:
                raise ValueError("fusion layer indices must be sorted and unique")
            if indices[0] < 0 or indices[-1] >= num_layers:
                raise ValueError("fusion layer index is outside the vision backbone")
            self.fusion_layer_indices = indices
        else:
            # Backward-compatible fallback for existing 2B configurations.
            indices = {
                max(
                    0,
                    min(
                        num_layers - 1,
                        round((stage + 1) * num_layers / num_fusion_stages) - 1,
                    ),
                )
