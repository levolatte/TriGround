...
def test_parallel_fusion_runs_before_each_deepstack_extraction():
    backbone = DummyDeepBackbone()
    vision = backbone.model.visual
    vision.deepstack_visual_indexes = [0, 1]
    vision.deepstack_merger_list = nn.ModuleList([DummyMerger(), DummyMerger()])
    model = MultiModalGrounder(
        backbone,
        adapter_channels=8,
        fusion_type="parallel_backbone",
        modality_dropout=0.0,
        parallel_fusion_stages=2,
        parallel_fusion_layer_indices=(0, 1),
    )
    events = []
    handles = []
    for index in (0, 1):
        handles.append(
            model.fusion.stage_fusions[str(index)].register_forward_hook(
                lambda _module, _inputs, _output, index=index: events.append(f"fusion-{index}")
            )
        )
        handles.append(
            vision.deepstack_merger_list[index].register_forward_hook(
                lambda _module, _inputs, _output, index=index: events.append(f"deepstack-{index}")
            )
        )
    try:
        model(**_batch())
